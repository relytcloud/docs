...
Relyt 使用标准的 HTTP 响应码来指示 API 请求的结果。一般来说，`2xx` 范围内的代码表示请求成功；`4xx` 范围内的代码表示客户端错误（例如，参数无效）；`5xx` 范围内的代码表示服务器端错误（此类错误较少发生）。
...
| `500`、`502`、`503`、`504` | Server Errors | Relyt AI 端发生错误（较少见）。 |
...
| `internal_server_error` | 内部服务器错误，表示处理请求时发生了 Relyt AI 端的问题，可能是临时问题、Bug 或系统故障。 |
| `idempotency_error` | 幂等性错误，同一幂等密钥被用于不同的 API 接口或参数集时发生冲突。 |
| `rate_limit_error` | 限速错误，您所在的团队超出了每秒 20 次 API 请求的上限。 |
...
| `300004` | Invalid file extension | 200 | 文件扩展名不受 Relyt AI 支持。支持的扩展名包括 **.csv**、**.tsv**、**.md**、**.mdx**、**.json**、**.txt**、**.pdf**、**.pptx**、**.ppt**、**.doc**、**.docx**、**.xls** 和 **.xlsx**。 |
